...
public enum OssFileEncodingType {
	 ...
	 * @param bom
	 */
	private OssFileEncodingType(int length, int bom, int carriageReturnChar, int lineFeedChar) {
		this.length = length;
		this.bom = bom;
	...
	public static OssFileEncodingType guessEncodingType(String encoding) {

		OssFileEncodingType encodingType;

		if (Const.isEmpty(encoding)) {
			encodingType = OssFileEncodingType.SINGLE;
		} else if (encoding.startsWith("UnicodeBig") || encoding.equals("UTF-16BE")) {
			encodingType = OssFileEncodingType.DOUBLE_BIG_ENDIAN;
		} else if (encoding.startsWith("UnicodeLittle") || encoding.equals("UTF-16LE")) {
			encodingType = OssFileEncodingType.DOUBLE_LITTLE_ENDIAN;
		} else if (encoding.equals("UTF-16")) {
			encodingType = OssFileEncodingType.DOUBLE_BIG_ENDIAN; // The default, no BOM
		} else {
			encodingType = OssFileEncodingType.SINGLE;
		}
