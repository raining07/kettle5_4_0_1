...
package org.pentaho.di.trans.steps.oss.fileinput;
 ...
 * @since 4-apr-2003
 */
public class OssFileInput extends BaseStep implements StepInterface {
	...
	private OssFileInputMeta meta;

	private OssFileInputData data;

	private long lineNumberInFile;

	public OssFileInput(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
			Trans trans) {
	...
	public static final String getLine(LogChannelInterface log, InputStreamReader reader, int formatNr,
		...
		OssFileEncodingType type = OssFileEncodingType.guessEncodingType(reader.getEncoding());
	...
	public static final String getLine(LogChannelInterface log, InputStreamReader reader,
			OssFileEncodingType encodingType, int formatNr, StringBuilder line) throws KettleFileException {
		int c = 0;
		line.setLength(0);
		try {
			switch (formatNr) {
			case OssFileInputMeta.FILE_FORMAT_DOS:
				while (c >= 0) {
					c = reader.read();
					...
					if (encodingType.isReturn(c) || encodingType.isLinefeed(c)) {
						...
						if (!encodingType.isReturn(c) && !encodingType.isLinefeed(c)) {
							...
							throw new KettleFileException(
									BaseMessages.getString(PKG, "OssFileInput.Log.SingleLineFound"));
						}
				...
				break;
			case OssFileInputMeta.FILE_FORMAT_UNIX:
				while (c >= 0) {
					c = reader.read();
				...
				break;
			case OssFileInputMeta.FILE_FORMAT_MIXED:
			...
			if (line.length() == 0) {
				throw new KettleFileException(
						BaseMessages.getString(PKG, "OssFileInput.Log.Error.ExceptionReadingLine", e.toString()), e);
			}
			return line.toString();
		}
		if (line.length() > 0) {
	...
	@Deprecated
	public static final String[] guessStringsFromLine(LogChannelInterface log, String line, OssFileInputMeta inf,
			String delimiter) throws KettleException {
		return guessStringsFromLine(new Variables(), log, line, inf, delimiter,
				StringUtil.substituteHex(inf.getEnclosure()), StringUtil.substituteHex(inf.getEscapeCharacter()));
	}

	public static final String[] guessStringsFromLine(VariableSpace space, LogChannelInterface log, String line,
			OssFileInputMeta inf, String delimiter, String enclosure, String escapeCharacter) throws KettleException {
		...
		try {
			...
			if (inf.getFileType().equalsIgnoreCase("CSV")) {
				...
				while (pos < length) {
					...
					if (len_encl > 0 && line.substring(from, from + len_encl).equalsIgnoreCase(enclosure)) {
						if (log.isRowLevel()) {
							log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
									BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRow",
						...
						if (log.isRowLevel()) {
							log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
									BaseMessages.getString(PKG, "OssFileInput.Log.EndOfEnclosure", "" + p));
						}
					} else {
						encl_found = false;
						boolean found = false;
					...
					if (encl_found) {
						...
						if (log.isRowLevel()) {
							log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
									BaseMessages.getString(PKG, "OssFileInput.Log.EnclosureFieldFound", "" + pol));
						}
					} else {
						pol = line.substring(from, next);
						if (log.isRowLevel()) {
							log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
									BaseMessages.getString(PKG, "OssFileInput.Log.NormalFieldFound", "" + pol));
						}
					}
				...
				if (pos == length) {
					if (log.isRowLevel()) {
						log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
								BaseMessages.getString(PKG, "OssFileInput.Log.EndOfEmptyLineFound"));
					}
					strings.add("");
				}
			} else {
				...
				for (int i = 0; i < inf.getInputFields().length; i++) {
					OssFileInputField field = inf.getInputFields()[i];
			...
			throw new KettleException(
					BaseMessages.getString(PKG, "OssFileInput.Log.Error.ErrorConvertingLine", e.toString()), e);
		}

		return strings.toArray(new String[strings.size()]);
	}

	public static final String[] convertLineToStrings(LogChannelInterface log, String line, OssFileMetaInterface inf,
			...
			if (inf.getFileType().equalsIgnoreCase("CSV")) {
				...
				while (pos < length) {
					...
					if (len_encl > 0 && line.substring(from, from + len_encl).equalsIgnoreCase(enclosure)) {
						if (log.isRowLevel()) {
							log.logRowlevel(BaseMessages.getString(PKG, "OssFileInput.Log.ConvertLineToRowTitle"),
									BaseMessages.getString(PKG, "OssFileInput.Log.Encloruse",
											line.substring(from, from + len_encl)));
						}
						encl_found = true;
						int p = from + len_encl;
					...
					try {
						...
						// This happens when you have less header values specified than there are actual
						// values in the rows.
	...
	/**
	 * @deprecated Use
	 *             {@link #convertLineToRow(LogChannelInterface, OssFileLine, InputFileMetaInterface, Object[], int, RowMetaInterface,RowMetaInterface, String, long, String, String, String, FileErrorHandler, boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean, String, String, boolean, Date, String, String, String, long)}
	 *             instead.
	 */
	@Deprecated
	public static final Object[] convertLineToRow(LogChannelInterface log, OssFileLine textFileLine,
			OssFileMetaInterface info, RowMetaInterface outputRowMeta, RowMetaInterface convertRowMeta, String fname,
		...
		return convertLineToRow(log, textFileLine, info, null, 0, outputRowMeta, convertRowMeta, fname, rowNr,
				delimiter, StringUtil.substituteHex(info.getEnclosure()),
				StringUtil.substituteHex(info.getEscapeCharacter()), errorHandler, addShortFilename, addExtension,
				addPath, addSize, addIsHidden, addLastModificationDate, addUri, addRootUri, shortFilename, path, hidden,
				modificationDateTime, uri, rooturi, extension, size);
	}

	public static final Object[] convertLineToRow(LogChannelInterface log, OssFileLine textFileLine,
			OssFileMetaInterface info, Object[] passThruFields, int nrPassThruFields, RowMetaInterface outputRowMeta,
		...
		try {
			...
			String[] strings = convertLineToStrings(log, textFileLine.line, info, delimiter, enclosure,
					escapeCharacter);
			int shiftFields = (passThruFields == null ? 0 : nrPassThruFields);
			for (fieldnr = 0; fieldnr < nrfields; fieldnr++) {
				OssFileInputField f = info.getInputFields()[fieldnr];
				...
				if (fieldnr < strings.length) {
					...
					try {
						...
						String message = BaseMessages.getString(PKG, "OssFileInput.Log.CoundNotParseField",
								valueMeta.toStringMeta(), "" + pol, valueMeta.getConversionMask(), "" + rowNr);

						if (info.isErrorIgnored()) {
							log.logDetailed(fname, BaseMessages.getString(PKG, "OssFileInput.Log.Warning") + ": "
			...
			throw new KettleException(BaseMessages.getString(PKG, "OssFileInput.Log.Error.ErrorConvertingLineText"),
					e);
		}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		data = (OssFileInputData) sdi;
		meta = (OssFileInputMeta) smi;
		Object[] r = null;
		boolean retval = true;
		...
		if (first) { // we just got started
			...
			if (meta.isAcceptingFilenames()) {
				...
				while (fileRow != null) {
					...
					if (idx < 0) {
						...
						if (idx < 0) {
							logError(BaseMessages.getString(PKG, "OssFileInput.Log.Error.UnableToFindFilenameField",
					...
					try {
						...
						logError(BaseMessages.getString(PKG, "OssFileInput.Log.Error.UnableToCreateFileObject",
								fileValue), e);
					}
				...
				if (data.getFiles().nrOfFiles() == 0) {
					if (log.isDetailed()) {
						logDetailed(BaseMessages.getString(PKG, "OssFileInput.Log.Error.NoFilesSpecified"));
					}
					setOutputDone();
					return false;
				}
			...
			if (!data.doneReading) {
				...
				for (int i = 0; i < repeats && !data.doneReading; i++) {
					String line = getLine(log, data.isr, data.encodingType, data.fileFormatType,
					...
					if (line != null) {
						...
						if (filterOK) {
							...
							data.lineBuffer.add(new OssFileLine(line, lineNumberInFile, data.file));
						} else {
							if (isFilterLastLine) {
		...
		/*
		 * If the buffer is empty: open the next file. (if nothing in there, open the
		 * next, etc.)
		 */
		 ...
		 */
		OssFileLine textLine = data.lineBuffer.get(0);
		incrementLinesInput();
		lineNumberInFile++;
		...
		if (meta.isLayoutPaged()) {
			...
			if (!data.doneWithHeader && data.pageLinesRead == 0) // We are reading header lines
			{
				...
				if (data.pageLinesRead < meta.getNrLinesPerPage()) {
					...
					r = convertLineToRow(log, textLine, meta, data.currentPassThruFieldsRow, data.nrPassThruFields,
							data.outputRowMeta, data.convertRowMeta, data.filename, useNumber, data.separator,
							data.enclosure, data.escapeCharacter, data.dataErrorLineHandler, data.addShortFilename,
							data.addExtension, data.addPath, data.addSize, data.addIsHidden,
							data.addLastModificationDate, data.addUri, data.addRootUri, data.shortFilename, data.path,
							data.hidden, data.lastModificationDateTime, data.uriName, data.rootUriName, data.extension,
							data.size);
					if (r != null) {
						putrow = true;
					}
					...
					if (!meta.hasFooter() && (data.pageLinesRead == meta.getNrLinesPerPage())) {
						/*
						 * OK, we are done reading the footer lines, start again on 'next page' with the
						 * header
						 */
			...
			if (!data.doneWithHeader) { // We are reading header lines
				...
				/*
				 * IF we are done reading and we have a footer AND the number of lines in the
				 * buffer is smaller then the number of footer lines THEN we can remove the
				 * remaining rows from the buffer: they are all footer rows.
				 */
				if (data.doneReading && meta.hasFooter() && data.lineBuffer.size() < meta.getNrFooterLines()) {
					...
					if (data.filePlayList.isProcessingNeeded(textLine.file, textLine.lineNumber,
						...
						r = convertLineToRow(log, textLine, meta, data.currentPassThruFieldsRow, data.nrPassThruFields,
								...
								data.enclosure, data.escapeCharacter, data.dataErrorLineHandler, data.addShortFilename,
								data.addExtension, data.addPath, data.addSize, data.addIsHidden,
								data.addLastModificationDate, data.addUri, data.addRootUri, data.shortFilename,
	...
	private void rejectCurrentFile(String errorMsg) {
		if (StringUtils.isNotBlank(meta.getFileErrorField())
			...
			int errorFileIndex = (StringUtils.isBlank(meta.getFileErrorField())) ? -1
					: addValueMeta(rowMeta, this.environmentSubstitute(meta.getFileErrorField()));

			int errorMessageIndex = StringUtils.isBlank(meta.getFileErrorMessageField()) ? -1
	...
	/**
	 * Adds <code>String</code> value meta with given name if not present and
	 * returns index
	 *
	 * @param rowMeta
	 * @param fieldName
	 ...
	 *
	 * @param line
	 * @param isFilterLastLine (dummy input param, only set when return value is
	 *                         false)
	 * @return true when the line should be taken (when false, isFilterLastLine will
	 *         be set)
	 */
	...
	private boolean closeLastFile() {
		try {
			...
			if (data.filename != null) {
				...
				/*
				 * } else if ( sFileCompression != null && sFileCompression.equals( "Snappy" )
				 * && data.sis != null ) { data.sis.close(); }
				 */
				data.in.close();
				data.isr.close();
			...
			// XXX deinspanjer 2009-07-07: I'm stubbing this out. The bug was ancient and it
	...
	private boolean openNextFile() {
		...
		try {
			...
			data.encodingType = OssFileEncodingType.guessEncodingType(encoding);
			...
			/*
			 * OK, read a number of lines in the buffer: The header rows The nr rows in the
			 * page : optional The footer rows
			 */
			int bufferSize = 1;
			...
			if (meta.isLayoutPaged()) {
				for (int i = 0; i < meta.getNrLinesDocHeader(); i++) {
					// Just skip these...
					getLine(log, data.isr, data.encodingType, data.fileFormatType, data.lineStringBuilder); // header
																											// and
																											// footer:
																											// not
																											// wrapped
			...
			for (int i = 0; i < bufferSize && !data.doneReading; i++) {
				...
				if (line != null) {
					...
					if (!meta.hasHeader() || i >= meta.getNrHeaderLines()) {
						...
						boolean filterOK = checkFilterRow(line, isFilterLastLine);
						if (filterOK) {
							data.lineBuffer.add(new OssFileLine(line, lineNumberInFile, data.file)); // Store it in the
							// line buffer...
						} else {
			...
			String errorMsg = "Couldn't open file #" + data.filenr + " : " + data.file.getName().getFriendlyURI()
					+ " --> " + e.toString();
			logError(errorMsg);
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			...
			data.filterProcessor = new OssFileFilterProcessor(meta.getFilter());
			...
			if ((previousResult == null || resultFiles == null || resultFiles.size() == 0)
					&& data.getFiles().nrOfMissingFiles() > 0 && !meta.isAcceptingFilenames()
					&& !meta.isErrorIgnored()) {
				logError(BaseMessages.getString(PKG, "OssFileInput.Log.Error.NoFilesSpecified"));
				return false;
			}
			...
			// // value="";
			// }
			// String null_cmp = Const.rightPad(new StringBuilder(null_value),
			// pol.length());
	...
	private void initErrorHandling() {
		...
		if (meta.getLineNumberFilesDestinationDirectory() != null) {
			dataErrorLineHandlers.add(new FileErrorHandlerContentLineNumber(getTrans().getCurrentDate(),
					environmentSubstitute(meta.getLineNumberFilesDestinationDirectory()),
					meta.getLineNumberFilesExtension(), meta.getEncoding(), this));
		}
		if (meta.getErrorFilesDestinationDirectory() != null) {
			dataErrorLineHandlers.add(new FileErrorHandlerMissingFiles(getTrans().getCurrentDate(),
					environmentSubstitute(meta.getErrorFilesDestinationDirectory()), meta.getErrorLineFilesExtension(),
