...
@Step(id = "OssFileInput", image = "oss-fileinput.png", i18nPackageName = "org.pentaho.di.trans.steps.oss.fileinput", name = "OssFileinput.Name", description = "OssFileinput.TooltipDesc", categoryDescription = "i18n:org.pentaho.di.trans.steps.oss.fileinput:OssFileinput.Step.Category")
public class OssFileInputMeta extends BaseStepMeta implements StepMetaInterface, OssFileMetaInterface {
	...
	/**
	 * Flag indicating that a single line is wrapped onto one or more lines in the
	 * text file.
	 */
	...
	private OssFileInputField[] inputFields;
	 ...
	 */
	public OssFileInputField[] getInputFields() {
		return inputFields;
	}
	 ...
	 */
	public void setInputFields(OssFileInputField[] inputFields) {
	 ...
	 */
	public OssFileFilter[] getFilter() {
		return filter;
	}
	 ...
	 */
	public void setFilter(OssFileFilter[] filter) {
		this.filter = filter;
	}
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases, IMetaStore metaStore) throws KettleXMLException {
		try {
			...
			if (XMLHandler.getTagValue(stepnode, "filter") != null) {
				filter = new OssFileFilter[1];
				filter[0] = new OssFileFilter();
				...
				filter[0].setFilterPositive(
						YES.equalsIgnoreCase(XMLHandler.getTagValue(stepnode, "filter_is_positive")));
			} else {
				for (int i = 0; i < nrfilters; i++) {
					Node fnode = XMLHandler.getSubNodeByNr(filtersNode, "filter", i);
					filter[i] = new OssFileFilter();
					...
					if (filterString != null && filterString.startsWith(STRING_BASE64_PREFIX)) {
						filter[i].setFilterString(new String(
								Base64.decodeBase64(filterString.substring(STRING_BASE64_PREFIX.length()).getBytes())));
					} else {
			...
			lineNumberFilesDestinationDirectory = XMLHandler.getTagValue(stepnode,
	...
	public Object clone() {
		OssFileInputMeta retval = (OssFileInputMeta) super.clone();
		...
		for (int i = 0; i < nrfields; i++) {
			retval.inputFields[i] = (OssFileInputField) inputFields[i].clone();
		}

		for (int i = 0; i < nrfilters; i++) {
			retval.filter[i] = (OssFileFilter) filter[i].clone();
		}
	...
	public void allocate(int nrfiles, int nrfields, int nrfilters) {
		...
		inputFields = new OssFileInputField[nrfields];
		filter = new OssFileFilter[nrfilters];
	}
	...
	public void setDefault() {
		...
		for (int i = 0; i < nrfields; i++) {
			inputFields[i] = new OssFileInputField("field" + (i + 1), 1, -1);
		}
	...
	public void getFields(RowMetaInterface row, String name, RowMetaInterface[] info, StepMeta nextStep,
		...
		for (int i = 0; i < inputFields.length; i++) {
			OssFileInputField field = inputFields[i];
		...
		if (getUriField() != null && getUriField().length() > 0) {
			ValueMetaInterface v = new ValueMeta(space.environmentSubstitute(getUriField()),
					ValueMetaInterface.TYPE_STRING);
			v.setLength(100, -1);
			v.setOrigin(name);
			row.addValueMeta(v);
	...
	public String getXML() {
		...
		retval.append("      ")
				.append(XMLHandler.addTagValue("compression", (fileCompression == null) ? "None" : fileCompression));
		retval.append("    </file>").append(Const.CR);
		...
		for (int i = 0; i < filter.length; i++) {
			...
			retval.append("        ")
					.append(XMLHandler.addTagValue("filter_is_last_line", filter[i].isFilterLastLine(), false));
			retval.append("        ")
					.append(XMLHandler.addTagValue("filter_is_positive", filter[i].isFilterPositive(), false));
			retval.append("      </filter>").append(Const.CR);
		}
		retval.append("    </filters>").append(Const.CR);
	...
	public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step)
		...
		try {
			...
			for (int i = 0; i < filter.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i, "filter_is_last_line",
						filter[i].isFilterLastLine());
				rep.saveStepAttribute(id_transformation, id_step, i, "filter_is_positive",
						filter[i].isFilterPositive());
			}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
		...
		if (input.length > 0) {
			if (!isAcceptingFilenames()) {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG, "OssFileInputMeta.CheckResult.NoInputError"), stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
						BaseMessages.getString(PKG, "OssFileInputMeta.CheckResult.AcceptFilenamesOk"), stepMeta);
				remarks.add(cr);
			}
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG, "OssFileInputMeta.CheckResult.NoInputOk"), stepMeta);
			remarks.add(cr);
		}
		...
		if (textFileList.nrOfFiles() == 0) {
			if (!isAcceptingFilenames()) {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG, "OssFileInputMeta.CheckResult.ExpectedFilesError"), stepMeta);
				remarks.add(cr);
			}
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, BaseMessages.getString(PKG,
					"OssFileInputMeta.CheckResult.ExpectedFilesOk", "" + textFileList.nrOfFiles()), stepMeta);
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
			Trans trans) {
		return new OssFileInput(stepMeta, stepDataInterface, cnr, transMeta, trans);
	}

	public StepDataInterface getStepData() {
		return new OssFileInputData();
	}
	...
	public int getFileFormatTypeNr() {
		...
		if (getFileFormat().equalsIgnoreCase("DOS")) {
			...
			return OssFileInputMeta.FILE_FORMAT_UNIX;
		} else {
			return OssFileInputMeta.FILE_FORMAT_MIXED;
		}
	}
	...
	public int getFileTypeNr() {
		...
		if (getFileType().equalsIgnoreCase("CSV")) {
			return OssFileInputMeta.FILE_TYPE_CSV;
		} else {
			return OssFileInputMeta.FILE_TYPE_FIXED;
		}
	}
	 ...
	 * files into absolute paths OR it simply includes the resource in the ZIP file.
	 * For now, we'll simply turn it into an absolute path and pray that the file is
	 * on a shared drive or something like that.
	 *
	...
	public StepMetaInjectionInterface getStepMetaInjectionInterface() {
		return new OssFileInputMetaInjection(this);
	}
