...
/**
 * This takes care of the external metadata injection into the TextFileInputMeta
 * class
 *
 * @author Matt
 */
public class OssFileInputMetaInjection implements StepMetaInjectionInterface {
	...
	enum Entry implements StepMetaInjectionEntryInterface {
		...
		public static Entry[] getTopEntries() {
			return new Entry[] { FILE_TYPE, SEPARATOR, ENCLOSURE, ESCAPE_CHAR, BREAK_IN_ENCLOSURE, HEADER_PRESENT,
					...
					NR_DOC_HEADER_LINES, NR_LINES_PER_PAGE, COMPRESSION_TYPE, NO_EMPTY_LINES, INCLUDE_FILENAME,
					FILENAME_FIELD, INCLUDE_ROW_NUMBER, ROW_NUMBER_BY_FILE, ROW_NUMBER_FIELD, FILE_FORMAT, ENCODING,
					ROW_LIMIT, DATE_FORMAT_LENIENT, DATE_FORMAT_LOCALE, ACCEPT_FILE_NAMES, ACCEPT_FILE_STEP,
					ACCEPT_FILE_FIELD, PASS_THROUGH_FIELDS, ADD_FILES_TO_RESULT, FILE_SHORT_FILE_FIELDNAME,
					FILE_PATH_FIELDNAME, FILE_HIDDEN_FIELDNAME, FILE_LAST_MODIFICATION_FIELDNAME, FILE_URI_FIELDNAME,
					FILE_EXTENSION_FIELDNAME, FILE_SIZE_FIELDNAME, SKIP_BAD_FILES, FILE_ERROR_FIELD,
					FILE_ERROR_MESSAGE_FIELD, IGNORE_ERRORS, ERROR_COUNT_FIELD, ERROR_FIELDS_FIELD, ERROR_TEXT_FIELD,
		...
		public static Entry[] getAggEntries() {
			return new Entry[] { FIELD_NAME, FIELD_POSITION, FIELD_LENGTH, FIELD_TYPE, FIELD_IGNORE, FIELD_FORMAT,
					FIELD_TRIM_TYPE, FIELD_PRECISION, FIELD_DECIMAL, FIELD_GROUP, FIELD_CURRENCY, FIELD_REPEAT,
					FIELD_NULL_STRING, FIELD_IF_NULL };
		}
	}

	private final OssFileInputMeta meta;

	public OssFileInputMetaInjection(OssFileInputMeta meta) {
		this.meta = meta;
	}
	...
	public void injectStepMetadataEntries(List<StepInjectionMetaEntry> all) throws KettleException {
		...
		List<OssFileInputField> fields = new ArrayList<OssFileInputField>();
		List<OssFileFilter> filters = new ArrayList<OssFileFilter>();
			...
			case FILTERS:
				for (StepInjectionMetaEntry lookField : lookFields.getDetails()) {
					...
					if (fieldEntry == Entry.FILTER) {
						OssFileFilter filterLine = new OssFileFilter();
		...
		// Only change a list when you need to, don't clear/reset existing content if
		// you don't send new content.
		//
		if (fields.size() > 0) {
			meta.setInputFields(fields.toArray(new OssFileInputField[fields.size()]));
		}
		if (fileLines.size() > 0) {
		...
		if (filters.size() > 0) {
			meta.setFilter(filters.toArray(new OssFileFilter[filters.size()]));
		}
	...
	public List<StepInjectionMetaEntry> extractStepMetadataEntries() throws KettleException {
		...
		if (!Const.isEmpty(meta.getInputFields())) {
			for (OssFileInputField inputField : meta.getInputFields()) {
				StepInjectionMetaEntry fieldEntry = getEntry(Entry.FIELD);
				fieldsEntry.getDetails().add(fieldEntry);
	...
	public OssFileInputMeta getMeta() {
		return meta;
	}
}
