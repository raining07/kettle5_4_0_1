...
public class OssUploader extends BaseStep implements StepInterface {
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		meta = (OssUploaderMeta) smi;
		data = (OssUploaderData) sdi;
		...
		if (first) {
				...
				if (data.indexOfSourceFilePath < 0) {
					...
					if (data.indexOfSourceFilePath < 0) {
						throw new KettleException(BaseMessages.getString(PKG,
								"OssUploader.Exception.CouldnotFindSourceFilePathField", realSourceFilePath));
					}
				}
			}
			...
			if (!Const.isEmpty(meta.getSourceFileName())) {
				...
				if (data.indexOfSourceFileName < 0) {
					...
					if (data.indexOfSourceFileName < 0) {
						throw new KettleException(BaseMessages.getString(PKG,
								"OssUploader.Exception.CouldnotFindSourceFileNameField", realSourceFileName));
					}
				}
			}
		}
