 ...
 *
 */
public class OssFileInputField implements Cloneable, TextFileInputFieldInterface {
	private String name;
	...
	private static final String[] date_formats = new String[] { "yyyy/MM/dd HH:mm:ss.SSS", "yyyy/MM/dd HH:mm:ss",
			"dd/MM/yyyy", "dd-MM-yyyy", "yyyy/MM/dd", "yyyy-MM-dd", "yyyyMMdd", "ddMMyyyy", "d-M-yyyy", "d/M/yyyy",
			"d-M-yy", "d/M/yy", };

	private static final String[] number_formats = new String[] { "", "#", Const.DEFAULT_NUMBER_FORMAT, "0.00",
			"0000000000000", "###,###,###.#######", "###############.###############", "#####.###############%", };

	public OssFileInputField(String fieldname, int position, int length) {
		this.name = fieldname;
	...
	public OssFileInputField() {
		this(null, -1, -1);
	}

	public int compare(Object obj) {
		OssFileInputField field = (OssFileInputField) obj;
	...
	public void guessType() {
		...
		for (int i = 0; i < samples.length && isnumber; i++) {
			...
			if (field.length() > 0 && field.equalsIgnoreCase(nullString)) {
				...
				for (int x = 0; x < field.length() && isnumber; x++) {
					...
					if (!Character.isDigit(ch) && ch != '.' && ch != ',' && (ch != '-' || x > 0) && ch != 'E'
							&& ch != 'e' // exponential
					) {
						isnumber = false;
						numfmt_cnt = 0;
		...
		// If all sample strings are empty or represent NULL values we can't take a
		// number as type.
		if (numfmt_cnt > 0 && numnul != samples.length) {
			 ...
			 * type=ValueMetaInterface.TYPE_INTEGER; decimalSymbol=""; groupSymbol=""; }
			 */
	...
	public TextFileInputFieldInterface createNewInstance(String newFieldname, int x, int newlength) {
		return new OssFileInputField(newFieldname, x, newlength);
	}
}
