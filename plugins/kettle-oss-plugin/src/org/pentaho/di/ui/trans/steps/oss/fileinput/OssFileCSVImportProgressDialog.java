...
import org.pentaho.di.trans.steps.oss.fileinput.OssFileLine;
import org.pentaho.di.trans.steps.oss.fileinput.OssFileMetaInterface;
...
public class OssFileCSVImportProgressDialog {
	...
	private OssFileEncodingType encodingType;

	/**
	 * Creates a new dialog that will handle the wait while we're finding out what
	 * tables, views etc we can reach in the database.
	 */
	public OssFileCSVImportProgressDialog(Shell shell, OssFileMetaInterface meta, TransMeta transMeta,
		...
		this.encodingType = OssFileEncodingType.guessEncodingType(reader.getEncoding());
	...
	public String open() {
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				try {
					...
					throw new InvocationTargetException(e,
							BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Exception.ErrorScanningFile",
		...
		try {
			...
			new ErrorDialog(shell,
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.ErrorScanningFile.Title"),
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.ErrorScanningFile.Message"), e);
	...
	private String doScan(IProgressMonitor monitor) throws KettleException {
		if (samples > 0) {
			monitor.beginTask(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Task.ScanningFile"),
					samples + 1);
		} else {
			monitor.beginTask(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Task.ScanningFile"), 2);
		}
		...
		for (int i = 0; i < nrfields; i++) {
			OssFileInputField field = meta.getInputFields()[i];
		...
		OssFileMetaInterface strinfo = (OssFileMetaInterface) meta.clone();
		for (int i = 0; i < nrfields; i++) {
		...
		line = OssFileInput.getLine(log, reader, encodingType, fileFormatType, lineBuffer);
		fileLineNumber++;
		int skipped = 1;
		...
		while (!errorFound && line != null && (linenr <= samples || samples == 0) && !monitor.isCanceled()) {
			monitor.subTask(
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Task.ScanningLine", "" + linenr));
			if (samples > 0) {
				monitor.worked(1);
			}
			...
			Object[] r = OssFileInput.convertLineToRow(log, new OssFileLine(line, fileLineNumber, null), strinfo, null,
					0, outputRowMeta, convertRowMeta, meta.getFilePaths(transMeta)[0], rownumber, delimiter, enclosure,
					escapeCharacter, null, false, false, false, false, false, false, false, false, null, null, false,
		...
		monitor.worked(1);
		monitor.setTaskName(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Task.AnalyzingResults"));

		// Show information on items using a dialog box
		//
		StringBuilder message = new StringBuilder();
		message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.ResultAfterScanning",
				"" + (linenr - 1)));
		message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.HorizontalLine"));
		...
		for (int i = 0; i < nrfields; i++) {
			...
			message.append(
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.FieldNumber", "" + (i + 1)));

			message.append(
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.FieldName", field.getName()));
			message.append(
					BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.FieldType", field.getTypeDesc()));

			switch (field.getType()) {
			case ValueMetaInterface.TYPE_NUMBER:
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.EstimatedLength",
						(field.getLength() < 0 ? "-" : "" + field.getLength())));
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.EstimatedPrecision",
						field.getPrecision() < 0 ? "-" : "" + field.getPrecision()));
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberFormat",
						field.getFormat()));

				if (!evaluationResults.isEmpty()) {
					if (evaluationResults.size() > 1) {
						message.append(
								BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.WarnNumberFormat"));
					}
					...
					for (StringEvaluationResult seResult : evaluationResults) {
						...
						message.append(
								BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberFormat2", mask));
						message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.TrimType",
								seResult.getConversionMeta().getTrimType()));
						message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberMinValue",
								seResult.getMin()));
						message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberMaxValue",
								seResult.getMax()));
						...
						try {
							...
							message.append(
									BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberExample",
											mask, seResult.getMin(), Double.toString(mn)));
						} catch (Exception e) {
							if (log.isDetailed()) {
								log.logDetailed("This is unexpected: parsing [" + seResult.getMin() + "] with format ["
										+ mask + "] did not work.");
							}
						}
					}
				}
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.NumberNrNullValues",
						"" + nrnull[i]));
				break;
			case ValueMetaInterface.TYPE_STRING:
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.StringMaxLength",
						"" + field.getLength()));
				message.append(
						BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.StringMinValue", minstr[i]));
				message.append(
						BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.StringMaxValue", maxstr[i]));
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.StringNrNullValues",
						"" + nrnull[i]));
				break;
			case ValueMetaInterface.TYPE_DATE:
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.DateMaxLength",
						field.getLength() < 0 ? "-" : "" + field.getLength()));
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.DateFormat",
						field.getFormat()));
				if (dateFormatCount[i] > 1) {
					message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.WarnDateFormat"));
				}
				if (!Const.isEmpty(minstr[i])) {
					for (int x = 0; x < Const.getDateFormats().length; x++) {
						if (dateFormat[i][x]) {
							message.append(BaseMessages.getString(PKG,
									"OssFileCSVImportProgressDialog.Info.DateFormat2", Const.getDateFormats()[x]));
							Date mindate = minDate[i][x];
							Date maxdate = maxDate[i][x];
							message.append(BaseMessages.getString(PKG,
									"OssFileCSVImportProgressDialog.Info.DateMinValue", mindate.toString()));
							message.append(BaseMessages.getString(PKG,
									"OssFileCSVImportProgressDialog.Info.DateMaxValue", maxdate.toString()));

							daf2.applyPattern(Const.getDateFormats()[x]);
							try {
								Date md = daf2.parse(minstr[i]);
								message.append(
										BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.DateExample",
				...
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.DateNrNullValues",
						"" + nrnull[i]));
				break;
			default:
				break;
			...
			if (nrnull[i] == linenr - 1) {
				message.append(BaseMessages.getString(PKG, "OssFileCSVImportProgressDialog.Info.AllNullValues"));
			}
